...
open class NetStream: NSObject {
    ...
    open func appendVideoSampleBufferSync(_ sampleBuffer: CMSampleBuffer) {
        mixer.videoIO.appendSampleBufferWithoutDraw(sampleBuffer)
    }

    open func drawVideoImageSync(_ image: CIImage) {
        mixer.videoIO.drawImage(image)
    }

    open func runVideoLockQueueAsync(foo: @escaping () -> Void) {
        mixer.videoIO.lockQueue.async {
            foo()
        }
    }

    open func registerEffect(video effect: VisualEffect, index: Int) -> Bool {
        return mixer.videoIO.registerEffect(effect, index: index)
    }
